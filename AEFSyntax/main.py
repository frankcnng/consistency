	...
	for str_file in files:
		if (str_file.endswith(".xlsx")):
			if str_file.endswith(".syntax_checked.xlsx"):
				...
				str_duplicate	= str_head + ".duplicate.xlsx"
				...
				if (is_valid):
					...
					if (os.path.exists(dst_path)):
						dst_path	= duplicate_dir + str_duplicate
						shutil.move(src_path, dst_path)							# move source to duplicate directory
...
def update_submission_status(path, str_error):
	"""	Update the worksheet status for the submission file at 'path',
		set the error message to 'str_error'.
	"""
	workbook	= load_workbook(path, data_only=True)
	worksheet	= workbook["Table 1 Submission"]
	worksheet.cell(9, 3, value=str_error + ' ' + time.strftime("%Y-%m-%dT%H:%M:%S", time.gmtime()) + 'Z')
	workbook.save(path)
